"""大坝剖面图 重试（同图同提示词，控制变量实验）：判断标签错位是逐次波动还是系统性误读。"""
import base64, json, time
from pathlib import Path
import requests

OUT = Path("/opt/wangjz/ragflow-import/out/vision_qwen27b_test")
IMG = Path("/home/scada/SmartTwinRes-skills/pdfs/05-基础数据与曲线/02-大坝剖面图.jpg")
...
for attempt in (1, 2, 3):
    try:
        ...
        (OUT / f"02-大坝剖面图_retry{tag}.txt").write_text(text, encoding="utf-8")
        print(json.dumps({"ok": True, "seconds": round(time.time() - t0, 1), "chars": len(text), "file": f"02-大坝剖面图_retry{tag}.txt"}, ensure_ascii=False))
        break
