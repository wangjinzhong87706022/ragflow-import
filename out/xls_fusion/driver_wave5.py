#!/usr/bin/env python3
"""批次四 driver：process（引擎全护栏）→ ES 回填 tag_feas → 引擎同款 verify 重验。
...
批次四特检：单目标 水位库容曲线推求.xls（523 块 F1 之最→12 块），无枚举扩、
无纠偏项（year None→2008 裁定点随 spec 落库）；本簿问句面 2008/2007 token 混杂，
不满足 P2-9b 补第三维前提，维持两维词表。
ES 索引 ragflow_72fc3f62…（ds3），scope=docnm_kwd。

用法：RAGFLOW_API_KEY=… python3 driver_wave5.py
"""
import json
import sys
