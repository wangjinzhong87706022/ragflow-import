#!/usr/bin/env python3
"""批次二 driver：process（引擎全护栏）→ ES 回填 tag_feas → 引擎同款 verify 重验。
...
批次二特检：本批三份旧壳词表含 历年统计 维（较大洪水统计表系正主不动，
本批三份迁移后统一两维）。ES 索引 ragflow_72fc3f62…（ds3），scope=docnm_kwd。

用法：RAGFLOW_API_KEY=… python3 driver_wave3.py
"""
import json
import sys
...
from rollout_xls_shell3 import CORPUS, ARCHIVE, STATE, MANIFEST, TARGETS, load_questions  # noqa: E402
